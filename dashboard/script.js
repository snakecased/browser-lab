...
async function stopSession(id) {
    ...
    if (window.whipResources && window.whipResources[id]) {
        try {
            await fetch(window.whipResources[id], {
                method: "DELETE",
            });
            console.log(
                "WHIP: Terminated resource:",
                window.whipResources[id],
            );
            delete window.whipResources[id];
        } catch (err) {
            console.error(
                "WHIP: Failed to terminate resource:",
                err,
            );
        }
    }
...
async function startWebRTC(sessionId) {
    ...
    dc.onmessage = (e) => {
        if (typeof e.data === "string") {
            ...
            window.currentFrame.buffer.set(
                chunk,
                window.currentFrame.receivedSize,
            );
            window.currentFrame.receivedSize += chunk.length;

            if (
                window.currentFrame.receivedSize >=
                window.currentFrame.totalSize
            ) {
                // Frame complete
                const blob = new Blob(
                    [window.currentFrame.buffer],
                    { type: "image/jpeg" },
                );
    ...
    pc.onicegatheringstatechange = () =>
        console.log(
            "WHIP: ICE Gathering State:",
            pc.iceGatheringState,
        );
    pc.onsignalingstatechange = () =>
        console.log("WHIP: Signaling State:", pc.signalingState);
    ...
    await new Promise((resolve) => {
        if (pc.iceGatheringState === "complete") {
            ...
            // Timeout after 10 seconds to send whatever we have
            setTimeout(() => {
                console.log(
                    "WHIP: ICE gathering timed out, proceeding with gathered candidates",
                );
                pc.removeEventListener(
                    "icegatheringstatechange",
                    checkState,
